...
void ab_checar_filhos_binaria(NO *noRaiz, bool *binaria);
void ab_checar_filhos_debusca(NO *noRaiz, bool *deBusca);

int ab_checar_busca(AB *T){
  if(T == NULL) return ERRO;

  bool deBusca = true;
  ab_checar_filhos_debusca(T->noRaiz, &deBusca);
...
void ab_checar_filhos_debusca(NO *noRaiz, bool *deBusca){
  ...
  ab_checar_filhos_debusca(noRaiz->noEsq, deBusca);
  ab_checar_filhos_debusca(noRaiz->noDir, deBusca);

  if((noRaiz->noEsq == NULL) || (noRaiz->noDir == NULL)) return;
  if(item_get_chave(noRaiz->noEsq->item) > item_get_chave(noRaiz->noDir->item)) *deBusca = false;
...
int ab_estritamente_binaria (AB *T){
  ...
  bool estrtBin = true;
  ab_checar_filhos_binaria(T->noRaiz, &estrtBin);
...
void ab_checar_filhos_binaria(NO *noRaiz, bool *binaria){
  ...
  ab_checar_filhos_binaria(noRaiz->noEsq, binaria);
  ab_checar_filhos_binaria(noRaiz->noDir, binaria);
