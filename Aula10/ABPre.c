...
void ab_imprimir_preordem(NO *noRaiz);
void ab_imprimir_emordem(NO *noRaiz);
void ab_imprimir_posordem(NO *noRaiz);
...
void ab_imprimir(AB *arv, int ordem){
  ...
  switch(ordem){
    case PRE_ORDEM:{
      ab_imprimir_preordem(arv->noRaiz);
      break;
    }
    case EM_ORDEM:{
      ab_imprimir_emordem(arv->noRaiz);
      break;
    }
    case POS_ORDEM:{
      ab_imprimir_posordem(arv->noRaiz);
      break;
    }
  }
}
...
void ab_imprimir_preordem(NO *noRaiz){
  if(noRaiz != NULL){
    ...
    ab_imprimir_preordem(noRaiz->noEsq);
    ab_imprimir_preordem(noRaiz->noDir);
  }
  return;
}
...
void ab_imprimir_emordem(NO *noRaiz){
  if(noRaiz != NULL){
    ab_imprimir_emordem(noRaiz->noEsq);
    printf("%d ", noRaiz->chave);
    ab_imprimir_emordem(noRaiz->noDir);
  }
  return;
}
...
void ab_imprimir_posordem(NO *noRaiz){
  if(noRaiz != NULL){
    ab_imprimir_posordem(noRaiz->noEsq);
    ab_imprimir_posordem(noRaiz->noDir);
    printf("%d ", noRaiz->chave);
  }
  return;
}
